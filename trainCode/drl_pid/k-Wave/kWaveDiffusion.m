...
classdef kWaveDiffusion < handle
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    methods
        ...
        function [e_drl,T_scale,mhz,mask_52] = takeTimeStep(obj, Nt, dt, t_once, end_rot, T_color, tumor_boundary,center_to_boundary,tt_center,W, rot_index,e_T,flag_50,mask_52,agent_rot,set_T)
            ...
            % temperature, so can be computed once and re-used)           % check if perfusion term is used
            ...
            Q_all{1} =  load(fullfile(dataRoot, 'Q_1600_005mm_down_400_02mm_2mhz.mat')).Q /2;
            Q_all{2} =  load(fullfile(dataRoot, 'Q_1600_005mm_down_400_02mm_3mhz.mat')).Q /2;
            Q_all{3} =  load(fullfile(dataRoot, 'Q_1600_005mm_down_400_02mm_6mhz.mat')).Q /2;
            Q_all{4} =  load(fullfile(dataRoot, 'Q_1600_005mm_down_400_02mm_10mhz.mat')).Q /2;
            %循环前准备
            ...
            time_dt = t_once / dt; %设置一次旋转的步长
             if dist <= 0.016
                ...
                % fprintf('dist = %.2fmm 换能器离肿瘤边界距离小于10mm,使用10mhz\n',dist*1000);
                mhz = 10;
             elseif dist <= 0.021
                rotated_Q =  imrotate(Q_all{3}* sqrt(W), rot_index ,"nearest","crop");
                % fprintf('dist = %.2fmm 换能器离肿瘤边界距离小于14mm,使用6mhz\n',dist*1000);
                mhz = 6;
             elseif dist <= 0.029
                rotated_Q =  imrotate(Q_all{2}* sqrt(W), rot_index ,"nearest","crop");
                % fprintf('dist = %.2fmm 换能器离肿瘤边界距离小于19mm,使用3mhz\n',dist*1000);
                mhz = 3;
             else
...
%             Q_initial = obj.Q; %初始声压矩阵
            ...
            for t_index = 1:Nt 
                ...
                if (flag_50 == 1 || flag_50 == 2) && obj.T(e_T(1,1),e_T(2,1)) >= set_T
                    T_scale = e_T;                    
                    indices = obj.T > mask_52;
                    % fprintf("温度达到52结束\n");
                    mask_52(indices) = obj.T(indices);
